...
const RoomDetails = () => {
  ...
  const {user} = useContext(AuthContext)
    const {_id,image,title,description,price,amenities,images,total_review} = roomdata
...
console.log(reviews);
  ...
  <div className=" card-body shadow-lg rounded-lg mb-5">
    ...
    <h3 className="text-3xl md:text-5xl font-bold text-blue-950  my-8">Reviews:</h3>
      ...
      {
        reviews.length==0 ?   <h3 className="text-3xl md:text-5xl font-bold text-blue-950 text-center"> No One Review Yet</h3> :
 ...
 <div className=" flex gap-5">
     <figure><img className="w-16 h-16 rounded-full" src={item.photoURL} alt="" /></figure>
     <div className="">
     <div className="flex gap-3 items-center">
     <h2 className="card-title mb-3 text-xl font-medium">{item.displayName}</h2>
 <p><small>Post on: {item.formattedDate}</small></p>
     </div>
     <div>Ratings:
     ...
     <h3 className="text-xl">{item.comment
}</h3> 
...
<div className="hero-content text-center text-neutral-content">
 ...
 {
    user ?  <Link to={`/rooms/booking/${_id}`}>
